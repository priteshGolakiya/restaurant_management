...
const StaffPage: React.FC = () => {
    ...
    const fetchStaff = async () => {
      try {
        const response = await axios.get<{ staff: StaffType[] }>(
          `${process.env.NEXT_PUBLIC_API_URL}/${summaryAPI.admin.staff.commonUlr}`,
          {
            headers: {
  ...
  const handleUpdate = async () => {
    ...
    try {
      await axios.put(
        `${process.env.NEXT_PUBLIC_API_URL}/api/admin/staff/${selectedStaff.userid}`,
        formData
      );
  ...
  const deleteUser = async (staff: StaffType) => {
    try {
      await axios.delete(
        `${process.env.NEXT_PUBLIC_API_URL}/api/admin/staff/${staff.userid}`
      );
